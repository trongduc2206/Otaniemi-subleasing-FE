...
const DefaultCheckBox = ( props,buttonLabel ) => {
    const onCheckClick = () => {
        props.handleToggle(props.isChecked);
    }
    return (
        <div className="defaultCheckBox">
            <div className={`box ${props.isChecked ? 'checked' : ''}`} onClick={onCheckClick}>
                {props.isChecked ? (
        ...
        <div className="defaultRadioButtonLabel">{props.buttonLabel}</div>
        </div>
    );
};
